...
procedure part_1 is
    ...
    function Get_Repeating_Item (C1 : Unbounded_String; C2 : Unbounded_String) return Character;

    function Get_Item_Priority (Item : Character) return Integer;

    function Get_Repeating_Item (C1 : Unbounded_String; C2 : Unbounded_String) return Character is
    begin
        for I in Positive range 1 .. Length(C1) loop
            for J in Positive range 1 .. Length(C2) loop
                if Element(C1, I) = Element(C2, J) then
                    return Element(C1, I);
                end if;
            end loop;
        end loop;
...
begin
    ...
    loop
        ...
        Item_temp := Get_Repeating_Item(Compartment_1, Compartment_2);
